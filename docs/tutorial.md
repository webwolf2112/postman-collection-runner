...
if (pm.response.code === 200) {
    ...
    // Also grab a random type for the bonus 4th call
...
// Real shared state, intentionally passed between requests. Since the
// Pokemon is picked randomly, this is the only reliable way to know
// which one we actually got back.
const actualPokemonName = pm.collectionVariables.get("randomPokemonName");

// --- Local variable (scoped to THIS script only) ---
// A plain JS const. It does NOT show up in the Variables tab and does NOT
// persist between requests — it's just a checklist this one test needs,
// not data flowing through the chain. A random pick will never match a
// hardcoded name, so this is a config value instead, not a guess.
const requiredFields = ["name", "id", "abilities", "types"];

pm.test("Response name matches the Pokemon that was actually requested", function () {
    const responseJson = pm.response.json();
    pm.expect(responseJson.name).to.eql(actualPokemonName);
});
...
**Tests: A Real Reason to Use a Local Variable**
```javascript
// --- Local variable (scoped to THIS script only) ---
...
// This IS worth passing between requests — it's the actual type we
// requested, picked randomly, and needed here to build the URL above.
